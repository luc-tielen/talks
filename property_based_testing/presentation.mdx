...
describe("addition", () => {
  it("is commutative", () => {
    assert(property(nat(), nat(), (a, b) => a + b === b + a));
  });
});
```
...
![](./images/shrinking_algorithm1.png)

![](./images/shrinking_algorithm2.png)

![](./images/shrinking_algorithm3.png)
...
const transaction = (fromAccount, toAccount, amount) => {
  if (fromAccount.amount > amount) {
    const fromUpdated = { amount: fromAccount.amount - amount };
    const toUpdated = { amount: toAccount.amount + amount };
  ...
  it("updates both accounts when transaction when completed", () => {
    fc.assert(
      fc.property(
        ...
        fc.integer(100, 200),
        (from, to, amount) => {
          const { fromUpdated, toUpdated } = transaction(from, to, amount);
          expect(fromUpdated.amount).to.eql(from.amount - amount);
          expect(toUpdated.amount).to.eql(to.amount + amount);
        }
      )
    );
  });
  ...
  it("aborts transaction when account has insufficient funds", () => {
    fc.assert(
      fc.property(
        ...
        fc.integer(300, 500),
        (from, to, amount) => {
          const { error } = transaction(from, to, amount);
...
## Async property example

```javascript
const fc = require("fast-check");

describe("Promise.all", () => {
  it("returns promise that resolves to array with results of promises", async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(fc.nat()), async values => {
        const promise = Promise.all(values.map(val => Promise.resolve(val)));
...
```notes
Can run any code in properties => also stuff like testing APIs, preact-render-spy, ... is possible!
Important:
